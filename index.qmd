...
#| echo: false
#| fig-cap: "Distribution of final balances highlighting the probability of exceeding the initial investment"
...
# Create bins for the histogram, covering the full range of final balances
bins = np.logspace(np.log10(df_final_balances['balance'].min()), np.log10(df_final_balances['balance'].max()), 40)

# Plot histogram for all final balances (grey)
ax.hist(df_final_balances['balance'], bins=bins, edgecolor='black', alpha=0.5, color='lightgrey', label='Losing Outcomes')

# Highlight the profitable outcomes (blue)
ax.hist(df_final_balances[df_final_balances['balance'] > INITIAL_BALANCE]['balance'],
        bins=bins, edgecolor='black', alpha=0.8, color='#228B22', label='Profitable Outcomes') # ForestGreen
...
ax.axvline(mean_balance, color='navy', linestyle=':', linewidth=2.5, label=f'Mean Final Balance (${mean_balance:,.0f})')
ax.axvline(median_balance, color='darkorange', linestyle=':', linewidth=2.5, label=f'Median Final Balance (${median_balance:,.0f})')

# Formatting
ax.set_xscale('log') # Use logarithmic scale for better distribution visibility
ax.set_title(f'Distribution of Final Balances After {NUM_YEARS} Years ({NUM_SIMULATIONS} Simulations)', fontsize=16, pad=20)
ax.set_xlabel('Final Account Balance ($)')
ax.set_ylabel('Frequency of Simulations')
ax.yaxis.set_major_formatter(FuncFormatter(lambda x, p: f'{int(x)}')) # For frequency, integer is fine
ax.xaxis.set_major_formatter(FuncFormatter(lambda x, p: f'${x:,.0f}')) # Currency format for x-axis
ax.grid(axis='y', linestyle='--', alpha=0.7)

# Adjust legend
handles, labels = ax.get_legend_handles_labels()
order = [2, 3, 4, 1, 0] # Reorder legend items for better storytelling
ax.legend([handles[idx] for idx in order],[labels[idx] for idx in order], loc='upper left')

# Add probability text annotation directly on the plot
ax.text(0.96, 0.95, f'P(Profit) = {prob_profit:.1%}',transform=ax.transAxes, fontsize=10,
        verticalalignment='top', horizontalalignment='right', bbox=dict(boxstyle='round,pad=0.5', fc='wheat', alpha=0.5))


# --- Print numeric result ---
# print(f"The estimated probability of ending with more than the initial ${INITIAL_BALANCE:,.0f} after {NUM_YEARS} years is: {prob_profit:.3f}")
```
